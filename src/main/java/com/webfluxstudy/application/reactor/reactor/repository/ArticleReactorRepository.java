...
import com.webfluxstudy.application.reactor.common.entity.ArticleEntity;
...
public class ArticleReactorRepository {
    ...
    public Flux<ArticleEntity> findAllByUserId(String userId) {
        ...
        Stream<ArticleEntity> articleEntityStream = articleEntities.stream()
                .filter(articleEntity -> articleEntity.getUserId().equals(userId));

        return Flux.fromStream(articleEntityStream);
...
//        });
    ...
    public Flux<ArticleEntity> findAllWithContext() {
        return Flux.deferContextual(contextView -> {
            Optional<UserEntity> optionalUser = contextView.getOrEmpty("user");
